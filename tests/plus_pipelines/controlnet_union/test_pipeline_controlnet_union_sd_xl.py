...
from diffusers.plus_models.controlnet_union import ControlNetModelUnion
...
class StableDiffusionXLControlNetPipelineFastTests(
    ...
    def test_controlnet_sdxl_lcm(self):
        ...
        sd_pipe = StableDiffusionXLControlNetUnionPipeline(**components)
    ...
    def test_controlnet_sdxl_two_mixture_of_denoiser_fast(self):
        components = self.get_dummy_components()
        pipe_1 = StableDiffusionXLControlNetUnionPipeline(**components).to(torch_device)
...
@slow
@require_torch_gpu
class ControlNetSDXLPipelineSlowTests(unittest.TestCase):#needs a lot of modifs, do we also need slow tests for union?
    def setUp(self):
        super().setUp()
        gc.collect()
...
###############################################
##---------------controlnetunion fast tests---------------------
class StableDiffusionXLControlNetUnionPipelineFastTests(
    IPAdapterTesterMixin,
    PipelineLatentTesterMixin,
    PipelineKarrasSchedulerTesterMixin,
    PipelineTesterMixin,
    ...
    def get_dummy_components(self, time_cond_proj_dim=None):
        ...
        torch.manual_seed(0)
        controlnet = ControlNetModelUnion(
            block_out_channels=(32, 64),
            layers_per_block=2,
            in_channels=4,
            down_block_types=("DownBlock2D", "CrossAttnDownBlock2D"),
