...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalControlNetMixin):
    ...
    def __init__(
        ...
        if controlnet_conditioning_embedding == "ControlNetConditioningEmbedding":
            ...
            self.controlnet_cond_embedding = UniControlTaskMOEEmbedding(
                time_embed_dim=time_embed_dim,
                model_channels=block_out_channels[0],
                conditioning_embedding_out_channels=conditioning_embedding_out_channels,
            )
            self.task_id_hypernet = UniControlTaskIDHypernet(
                time_embed_dim=time_embed_dim, cross_attention_dim=cross_attention_dim
            )
        else:
            raise ValueError(
                f"Unsupported value for controlnet_conditioning_embedding: {controlnet_conditioning_embedding}"
            )
    ...
    def forward(
        ...
        if isinstance(self.controlnet_cond_embedding, ControlNetConditioningEmbedding):
            ...
            controlnet_cond = self.controlnet_cond_embedding(
                x=sample, context=encoder_hidden_states, hint=controlnet_cond, task_id_emb=task_id_emb, task_id=task_id
            )
        else:
        ...
        if isinstance(self.controlnet_cond_embedding, ControlNetConditioningEmbedding):
            ...
            for down_block_res_sample, controlnet_block, task_hyperlayer in zip(
                down_block_res_samples, self.controlnet_down_blocks, self.task_id_layernet
            ):
                down_block_res_sample = modulated_conv2d(
                    down_block_res_sample,
                    controlnet_block.weight,
                    task_hyperlayer(task_id_emb).repeat(sample.shape[0], 1).detach(),
                )  # BS_Real = sample.shape[0] ?
