...
task_to_name = {
    ...
    "outpainting": "control_outpainting",
    "hedsketch": "control_hedsketch",
    "inpainting": "control_inpainting",
    "blur": "control_blur",
}
...
tasks_to_id = {
    ...
    "control_hedsketch": 7,
    "control_bbox": 8,
    "control_outpainting": 9,
    "control_grayscale": 10,
    "control_blur": 11,
...
class StableDiffusionUniControlPipeline(
    ...
    def check_inputs(
        ...
        if isinstance(task, list) and not set(task).issubset(task_to_name.keys()):
            raise ValueError(f"Values for `task` can only be {list(task_to_name.keys())}")
        elif isinstance(task, str) and task not in task_to_name:
            raise ValueError(f"Values for `task` can only be {list(task_to_name.keys())} but passed {task}")
        ...
        if isinstance(task, str):
            ...
            if isinstance(controlnet_conditioning_scale, list):
                if any(isinstance(i, list) for i in controlnet_conditioning_scale):
                    raise ValueError("A single batch of multiple conditionings are supported at the moment.")
            elif isinstance(controlnet_conditioning_scale, list) and len(controlnet_conditioning_scale) != len(task):
                raise ValueError(
            ...
            for i, t in enumerate(timesteps):
                ...
                for indiv_task_index, (indiv_task, indiv_image) in enumerate(zip(task, image)):
                    ...
                    down_samples, mid_sample = self.controlnet(
                        ...
                        conditioning_scale=cond_scales[indiv_task_index]
                        * controlnet_keep[
                            i
                        ],  # number or a list. controlnet vs multicontrolnet. For our case, always needs to be a number
                        guess_mode=guess_mode,
                        return_dict=False,
                    )
