...
class KolorsInpaintPipeline(DiffusionPipeline, StableDiffusionMixin, StableDiffusionXLLoraLoaderMixin, IPAdapterMixin):
    r"""
    ...
    def __call__(
        ...
        timesteps, num_inference_steps = self.get_timesteps(
                ...
                strength,
                device,
                denoising_start = self.denosing_start if denoising_value_valid(self.denoising_start) else None,
        )
        ...
        if not output_type == "latent":
            ...
            return KolorsPipelineOutput(images=latents)

        if not output_type == "latent":
            image = self.image_processor.postprocess(image, output_type=output_type)

        if padding_mask_crop is not None:
            image = [self.image_processor.apply_overlay(mask_image, origional_image, i, crops_coords) for i in image]
